...
from acc.models import AdTestType0, Licence, Record, UserProfile, Section
from form.models import *
from report.models import Report, Encode
from form.views import model_list
from ww.local_settings import DL_FTP_HOST, DL_FTP_USER, DL_FTP_PASSWD, domain_name, DEBUG
...
def send_file_ftp(ftp_obj, fill, reportn):
    fp = open(reportn, 'rb')
    ftp_obj.storbinary('STOR %s' % os.path.basename(fill), fp, 8192)

Section.objects.order_by('-eng_name')


#
# for model in model_list:
#     query = model[1].objects.all()
#     for obj in query:
#         # Heal the Report Properties
#         report_query = Report.objects.filter(record=obj.record)
#         if len(report_query) ==0:
#             report_instance = Report.objects.create(
#                 tt=AdTestType0.objects.get(type=(model[0] if model[0] != 'spo2' else 'PulseOximetry')),
#                 device=obj.device,
#                 request=obj.request, date=obj.date, user=obj.user, status=obj.status,
#                 record=obj.record,
#                 licence=obj.licence if model[1] != CantTest else Licence.objects.get(number=-1),
#                 is_recal=obj.is_recal,
#                 ref_record=obj.ref_record if model[1] != CantTest else Record.objects.get(number=-1),
#                 is_done=obj.is_done, totalcomment=obj.totalcomment)
#             report_instance.save()
#             print('Saved: ', str(obj.record.number))
        ...
        for obj in query:
            ...
            if len(encode_query) == 0:
                ...
                encode_instance = Encode.objects.create(
                    hospital=obj.devic1e.hospital, name=filename)
            ...
            if not item[0] in ftp.nlst():
                ftp.mkd(item[0])
            ftp.cwd(item[0])
            if '{}.pdf'.format(obj.licence.number) in ftp.nlst():
                # print(ftp.nlst())
                continue
            print(obj)
            with open('report_status.txt', 'a') as report_file:
                report_file.write(str(obj.record.number) + '\n')
                report_file.write(ftp.pwd() + '\n')
                report_file.close()
            data = []
            if item[1] != CantTest:
                ...
                if item[1] == MonitorSpo2_1:
                    ...
                    if 550 >= obj.s16_e1 >= 450:  # 0
                        data.append(1)
                    else:
                        data.append(0)
                    if 13.2 >= obj.s16_e2 >= 10.8:  # 1
                        data.append(1)
                    else:
                        data.append(0)
                    if obj.s16_1e1 != -1:  # 2
                        if (obj.s16_1e1 * 1.1) >= obj.s16_e3 >= (obj.s16_1e1 * 0.9):
                            data.append(1)
                        else:
                            data.append(0)
                    else:
                        data.append(2)
                    if obj.s16_1e2 != -1:  # 3
                        if (obj.s16_1e2 * 1.1) >= obj.s16_e4 >= (obj.s16_1e2 * 0.9):
                            data.append(1)
                        else:
                            data.append(0)
                    else:
                        data.append(2)
                    if 0.55 >= obj.s16_e5 >= 0.45:  # 4
                        data.append(1)
                    else:
                        data.append(0)
                    if 22 >= obj.s16_e6 >= 18:  # 5
                        data.append(1)
                    else:
                        data.append(0)
                    if obj.s16_1e3 != -1:  # 6
                        if (obj.s16_1e3 * 1.1) >= obj.s16_e7 >= (obj.s16_1e3 * 0.9):
                            data.append(1)
                        else:
                            data.append(0)
                    else:
                ...
                # ===================================Begin-FTP Stuf=================================================
                print('Saved: ', obj.record.number)
                if not DEBUG:
                    send_file_ftp(
                        ftp, '{}.pdf'.format(obj.licence.number), report_name)
                    print('Uploaded: ', obj.record.number)
                    os.remove(report_name)
                    print('Removed: ', obj.record.number)
                    print()
